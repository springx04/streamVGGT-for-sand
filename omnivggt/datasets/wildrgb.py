...
#
# --------------------------------------------------------
# Dataloader for preprocessed wildrgb dataset
# See datasets_preprocess/preprocess_wildrgb.py
# --------------------------------------------------------
...
def read_pose(datapath):
    poses = np.genfromtxt(datapath)
    poses = poses[:, 1:].reshape(-1, 4, 4)
    
    return poses

class Wildrgb(BaseStereoViewDataset):
    def __init__(self,
                 dataset_location='/mnt/disk3.8-4/datasets/wildrgbd',
                 use_cache = False,
                 dset='',
                 z_far = 50,
                 use_augs=False,
        ...
        print('loading Wildrgb dataset...')
        super().__init__(*args, **kwargs)
        self.dataset_label = 'Wildrgb'
        self.split = dset
        ...
        if self.use_cache:
            dataset_location = '/mnt/disk3.8-4/annotations/wildrgbd_annotations'
            ...
            for seq in self.sequences:
                ...
                sub_scenes = os.listdir(os.path.join(seq,"scenes"))
                if quick:
                    sub_scenes = sub_scenes[:1]
                for sub_seq in sub_scenes:
                    if sub_seq.startswith('.'):
                        continue
                    rgb_path = os.path.join(seq, 'scenes', sub_seq, 'rgb')
                    mask_path = os.path.join(seq, 'scenes', sub_seq, 'masks')
                    depth_path = os.path.join(seq, 'scenes', sub_seq, 'depth')
                    annotaions_file_path = os.path.join(seq, 'scenes', sub_seq, 'metadata')
                    ...
                    assert len(self.all_rgb_paths) == N and \
                        ...
                        len(self.all_depth_paths) == N, f"Number of images, depth maps, and annotations do not match in {seq}, {sub_seq}."
            ...
            # # 保存为 JSON 文件
            # os.makedirs(f'/mnt/disk3.8-4/annotations/wildrgbd_annotations/{dset}', exist_ok=True)
            # self._save_paths_to_json(self.all_rgb_paths, f'/mnt/disk3.8-4/annotations/wildrgbd_annotations/{dset}/rgb_paths.json')
            # self._save_paths_to_json(self.all_depth_paths, f'/mnt/disk3.8-4/annotations/wildrgbd_annotations/{dset}/depth_paths.json')
            # joblib.dump(self.all_extrinsic, f'/mnt/disk3.8-4/annotations/wildrgbd_annotations/{dset}/extrinsics.joblib')
            # joblib.dump(self.all_intrinsic, f'/mnt/disk3.8-4/annotations/wildrgbd_annotations/{dset}/intrinsics.joblib')
            # joblib.dump(self.rank, f'/mnt/disk3.8-4/annotations/wildrgbd_annotations/{dset}/rankings.joblib')
            print('found %d frames in %s (dset=%s)' % (len(self.full_idxs), dataset_location, dset))
    ...
    def _get_views(self, index, num, resolution, rng):
        ...
        for i in range(num):
            ...
            # depthmap = read_depth(depthpath,scale_adjustment=1000)
            depth = Image.open(depthpath)
            depthmap = np.array(depth, dtype=np.float32)
