...
#
# --------------------------------------------------------
# Dataloader for preprocessed co3d dataset
...
class Co3d(BaseStereoViewDataset):
    def __init__(self,
                 dataset_location='/mnt/disk3.8-4/datasets/co3d',
                 dset='',
                 use_cache=False,
                 use_augs=False,
                 top_k=256,
        ...
        print('loading Co3d dataset...')
        super().__init__(*args, **kwargs)

        # Initialize instance attributes
        self.dataset_label = 'Co3d'
        self.dset = dset
        self.top_k = top_k
        self.z_far = z_far
        ...
        if self.use_cache:
            dataset_location = '/mnt/disk3.8-4/annotations/co3dv2_annotations'
            ...
            all_mask_paths_file = os.path.join(dataset_location, dset, 'mask_paths.json')
            ...
            with open(all_mask_paths_file, 'r', encoding='utf-8') as file:
                self.all_mask_paths = json.load(file)
            ...
            self.all_mask_paths = [self.all_mask_paths[str(i)] for i in range(len(self.all_mask_paths))]
            ...
            self.max_depths = joblib.load(os.path.join(dataset_location, dset, 'max_depths.joblib'))
            ...
            for seq in self.sequences:
                ...
                if quick:
                    sub_scenes = sub_scenes[:1]
                for sub_seq in sub_scenes:
                    if sub_seq == "selected_seqs_test.json" or sub_seq == "selected_seqs_train.json":
                        continue

                    rgb_path = os.path.join(seq, sub_seq, 'images')
                    mask_path = os.path.join(seq, sub_seq, 'masks')
                    depth_path = os.path.join(seq, sub_seq, 'depths')
                    annotaions_file_path = os.path.join(seq, sub_seq, 'images')
                    ...
                    self.all_mask_paths.extend(sorted(glob.glob(os.path.join(mask_path, '*.png'))))
                    ...
                    for anno in seq_annotaions_path:
                        ...
                        self.max_depths.extend([camera_info['maximum_depth']])
            ...
            os.makedirs(f'annotations/co3dv2_annotations/{dset}', exist_ok=True)
            self._save_paths_to_json(self.all_rgb_paths, f'annotations/co3dv2_annotations/{dset}/rgb_paths.json')
            self._save_paths_to_json(self.all_depth_paths, f'annotations/co3dv2_annotations/{dset}/depth_paths.json')
            self._save_paths_to_json(self.all_mask_paths, f'annotations/co3dv2_annotations/{dset}/mask_paths.json')
            joblib.dump(self.all_extrinsic, f'annotations/co3dv2_annotations/{dset}/extrinsics.joblib')
            joblib.dump(self.all_intrinsic, f'annotations/co3dv2_annotations/{dset}/intrinsics.joblib')
            joblib.dump(self.rank, f'annotations/co3dv2_annotations/{dset}/rankings.joblib')
            joblib.dump(self.max_depths, f'annotations/co3dv2_annotations/{dset}/max_depths.joblib')
            print('found %d frames in %s (dset=%s)' % (len(self.full_idxs), dataset_location, dset))
    ...
    def _get_views(self, index, num, resolution, rng):
        ...
        max_depths = [self.max_depths[i] for i in full_idx]

        views = []
        for impath, depthpath, camera_pose, intrinsics, max_depth in zip(rgb_paths, depth_paths, camera_pose_list, intrinsics_list, max_depths):
            # Load and preprocess images
            rgb_image = imread_cv2(impath)
            depthmap = self._read_depthmap(depthpath, max_depth=max_depth)
