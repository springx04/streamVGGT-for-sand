...
#
# --------------------------------------------------------
# Dataloader for preprocessed bedlam dataset
# See datasets_preprocess/preprocess_bedlam.py
# --------------------------------------------------------
...
class Bedlam(BaseStereoViewDataset):
    def __init__(self,
                 dataset_location='/mnt/disk3.8-4/datasets/bedlam',
                 dset='',
                 use_cache=False,
                 use_augs=False,
                 top_k=128,
        ...
        if self.use_cache:
            dataset_location = '/mnt/disk3.8-4/annotations/bedlam_annotations'
            ...
            for seq in self.sequences:
                ...
                self.all_annotation_paths.extend(sorted(glob.glob(os.path.join(cam_path, '*.npz'))))
                ...
                all_annotations = sorted(glob.glob(os.path.join(cam_path, '*.npz')))
                all_extrinsic = []
                # load annotations
                for anno in all_annotations:
                    annotation = np.load(anno)
                    extrinsic = np.array(annotation['pose'], dtype=np.float32)
                    intrinsic = np.array(annotation['intrinsics'], dtype=np.float32)
                    all_extrinsic.append(extrinsic)
                    self.all_extrinsic.extend([extrinsic])
                    self.all_intrinsic.extend([intrinsic])
                all_extrinsic_numpy = np.array(all_extrinsic)
            ...
            os.makedirs(f'annotations/bedlam_annotations/{dset}', exist_ok=True)
            self._save_paths_to_json(self.all_rgb_paths, f'annotations/bedlam_annotations/{dset}/rgb_paths.json')
            self._save_paths_to_json(self.all_depth_paths, f'annotations/bedlam_annotations/{dset}/depth_paths.json')
            joblib.dump(self.all_extrinsic, f'annotations/bedlam_annotations/{dset}/extrinsics.joblib')
            joblib.dump(self.all_intrinsic, f'annotations/bedlam_annotations/{dset}/intrinsics.joblib')
            joblib.dump(self.rank, f'annotations/bedlam_annotations/{dset}/rankings.joblib')
            print('found %d frames in %s (dset=%s)' % (len(self.full_idxs), dataset_location, dset))
