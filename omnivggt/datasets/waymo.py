...
#
# --------------------------------------------------------
# Dataloader for preprocessed waymo dataset
...
class Waymo(BaseStereoViewDataset):
    def __init__(self,
                 dataset_location='/mnt/disk3.8-4/datasets/waymo',
                 use_cache = False,
                 dset='',
                 z_far = 655,
                 use_augs=False,
        ...
        if self.use_cache:
            dataset_location = '/mnt/disk3.8-4/annotations/waymo_annotations'
            ...
            for seq in self.sequences:
                ...
                rgb_path = seq
                depth_path = seq
                annotaions_file_path = seq
                num_frames = len(glob.glob(os.path.join(rgb_path, '*.jpg')))
                
                if num_frames < 24:
                    print(f"Skipping sequence {seq} due to insufficient frames ({num_frames})")
                    continue
                ...
                for anno in seq_annotaions_path:
                    ...
                    pose = np.array(camera_info['cam2world'],dtype=np.float32)
            ...
            # # 保存为 JSON 文件
            # os.makedirs(f'/mnt/disk3.8-4/annotations/waymo_annotations/{dset}', exist_ok=True)
            # self._save_paths_to_json(self.all_rgb_paths, f'/mnt/disk3.8-4/annotations/waymo_annotations/{dset}/rgb_paths.json')
            # self._save_paths_to_json(self.all_depth_paths, f'/mnt/disk3.8-4/annotations/waymo_annotations/{dset}/depth_paths.json')
            # joblib.dump(self.all_extrinsic, f'/mnt/disk3.8-4/annotations/waymo_annotations/{dset}/extrinsics.joblib')
            # joblib.dump(self.all_intrinsic, f'/mnt/disk3.8-4/annotations/waymo_annotations/{dset}/intrinsics.joblib')
            # joblib.dump(self.rank, f'/mnt/disk3.8-4/annotations/waymo_annotations/{dset}/rankings.joblib')
            print('found %d frames in %s (dset=%s)' % (len(self.full_idxs), dataset_location, dset))
