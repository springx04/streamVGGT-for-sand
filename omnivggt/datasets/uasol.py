...
#
# --------------------------------------------------------
# Dataloader for preprocessed uasol dataset
# See datasets_preprocess/preprocess_uasol.py
# --------------------------------------------------------
...
class Uasol(BaseStereoViewDataset):
    def __init__(self,
                 dataset_location='/mnt/disk3.8-4/datasets/uasol',
                 use_cache = False,
                 dset='',
                 use_augs=False,
                 top_k = 256,
        ...
        print('loading uasol dataset...')
        super().__init__(*args, **kwargs)
        self.dataset_label = 'uasol'
        self.split = dset
        ...
        if self.use_cache:
            dataset_location = '/mnt/disk3.8-4/annotations/uasol_annotations'
            ...
            # # 保存为 JSON 文件
            os.makedirs(f'/mnt/disk3.8-4/annotations/uasol_annotations/{dset}', exist_ok=True)
            self._save_paths_to_json(self.all_rgb_paths, f'/mnt/disk3.8-4/annotations/uasol_annotations/{dset}/rgb_paths.json')
            self._save_paths_to_json(self.all_depth_paths, f'/mnt/disk3.8-4/annotations/uasol_annotations/{dset}/depth_paths.json')
            joblib.dump(self.all_extrinsic, f'/mnt/disk3.8-4/annotations/uasol_annotations/{dset}/extrinsics.joblib')
            joblib.dump(self.all_intrinsic, f'/mnt/disk3.8-4/annotations/uasol_annotations/{dset}/intrinsics.joblib')
            joblib.dump(self.rank, f'/mnt/disk3.8-4/annotations/uasol_annotations/{dset}/rankings.joblib')
            print('found %d frames in %s (dset=%s)' % (len(self.full_idxs), dataset_location, dset))
