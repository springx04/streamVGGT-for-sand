...
#
# --------------------------------------------------------
# Dataloader for preprocessed megadepth dataset
...
class MegaDepth(BaseStereoViewDataset):
    def __init__(self,
                 dataset_location='/mnt/disk3.8-4/datasets/megadepth',
                 use_cache = False,
                 dset='',
                 use_augs=False,
                 z_far = 1000,
        ...
        print('loading MegaDepth dataset...')
        ...
        if self.use_cache:
            dataset_location = '/mnt/disk3.8-4/annotations/megadepth_annotations'
            ...
            for seq in self.sequences:
                ...
                for sub_seq in sub_scenes:
                    ...
                    self.all_annotation_paths.extend(sorted(glob.glob(os.path.join(rgb_path, '*.npz'))))
                    ...
                    annotations = sorted(glob.glob(os.path.join(rgb_path, '*.npz')))
                    # load annotations   
                    extrinsics_seq = []                 
                    for anno in annotations:
                        camera_params = np.load(anno)
                        poses = np.array(camera_params['cam2world'],dtype=np.float32)
                        intrinsic =np.array(camera_params['intrinsics'],dtype=np.float32)
            ...
            # 保存为 JSON 文件
            os.makedirs('/mnt/disk3.8-4/annotations/megadepth_annotations', exist_ok=True)
            self._save_paths_to_json(self.all_rgb_paths, '/mnt/disk3.8-4/annotations/megadepth_annotations/rgb_paths.json')
            self._save_paths_to_json(self.all_depth_paths, '/mnt/disk3.8-4/annotations/megadepth_annotations/depth_paths.json')
            joblib.dump(self.all_extrinsic, '/mnt/disk3.8-4/annotations/megadepth_annotations/extrinsics.joblib')
            joblib.dump(self.all_intrinsic, '/mnt/disk3.8-4/annotations/megadepth_annotations/intrinsics.joblib')
            joblib.dump(self.rank, '/mnt/disk3.8-4/annotations/megadepth_annotations/rankings.joblib')  
            print('found %d frames in %s (dset=%s)' % (len(self.full_idxs), dataset_location, dset))
