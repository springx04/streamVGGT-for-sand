...
#
# --------------------------------------------------------
# Dataloader for preprocessed vkitti dataset
# See datasets_preprocess/preprocess_vkitti.py
# --------------------------------------------------------
...
class Vkitti(BaseStereoViewDataset):
    def __init__(self,
        ...
        if self.use_cache:
            dataset_location = '/mnt/disk3.8-4/annotations/vkitti_annotations'
            ...
            for seq in self.sequences:
                ...
                for sub_seq in sub_scenes:
                    sub_seq_path = os.path.join(seq, sub_seq)
                    sub_sub_scenes = os.listdir(sub_seq_path)
                    for sub_sub_seq in sub_sub_scenes:

                        rgb_path = os.path.join(seq, sub_seq, sub_sub_seq)
                        depth_path = os.path.join(seq, sub_seq, sub_sub_seq)
                        annotaions_file_path = os.path.join(seq, sub_seq, sub_sub_seq)
                        num_frames = len(glob.glob(os.path.join(rgb_path, '*.jpg')))
                        
                        if num_frames < 24:
                            print(f"Skipping sequence {sub_seq} in {seq} due to insufficient frames ({num_frames})")
                            continue
                        ...
                        self.all_depth_paths.extend(sorted(glob.glob(os.path.join(depth_path, '*depth.png'))))
            ...
            # # 保存为 JSON 文件
            os.makedirs(f'/mnt/disk3.8-4/annotations/vkitti_annotations/{dset}', exist_ok=True)
            self._save_paths_to_json(self.all_rgb_paths, f'/mnt/disk3.8-4/annotations/vkitti_annotations/{dset}/rgb_paths.json')
            self._save_paths_to_json(self.all_depth_paths, f'/mnt/disk3.8-4/annotations/vkitti_annotations/{dset}/depth_paths.json')
            joblib.dump(self.all_extrinsic, f'/mnt/disk3.8-4/annotations/vkitti_annotations/{dset}/extrinsics.joblib')
            joblib.dump(self.all_intrinsic, f'/mnt/disk3.8-4/annotations/vkitti_annotations/{dset}/intrinsics.joblib')
            joblib.dump(self.rank, f'/mnt/disk3.8-4/annotations/vkitti_annotations/{dset}/rankings.joblib')
            print('found %d frames in %s (dset=%s)' % (len(self.full_idxs), dataset_location, dset))
