...
#
# --------------------------------------------------------
# Dataloader for preprocessed dynamic replica dataset
# See datasets_preprocess/preprocess_dynamic_replica.py
# --------------------------------------------------------
...
class Dynamic_Replica(BaseStereoViewDataset):
    def __init__(self,
                 dataset_location='datasets/dynamic_replica',
                 dset='train',
                 use_cache=True,
                 use_augs=False,
                 top_k=256,
        ...
        if self.use_cache:
            dataset_location = "/mnt/disk3.8-4/annotations/dynamic_replica_annotations"
            ...
            for seq in self.sequences:
                ...
                for sub_seq in sub_scenes:
                    ...
                    annotaions_file_path = os.path.join(seq, sub_seq, 'cam')
            ...
            os.makedirs(f'annotations/dynamic_replica_annotations/{dset}', exist_ok=True)
            self._save_paths_to_json(self.all_rgb_paths, f'annotations/dynamic_replica_annotations/{dset}/rgb_paths.json')
            self._save_paths_to_json(self.all_depth_paths, f'annotations/dynamic_replica_annotations/{dset}/depth_paths.json')
            self._save_paths_to_json(self.all_annotation_paths, f'annotations/dynamic_replica_annotations/{dset}/annotation_paths.json')
            joblib.dump(self.rank, f'annotations/dynamic_replica_annotations/{dset}/rankings.joblib')
            joblib.dump(self.all_extrinsic, f'annotations/dynamic_replica_annotations/{dset}/extrinsics.joblib')
            joblib.dump(self.all_intrinsic, f'annotations/dynamic_replica_annotations/{dset}/intrinsics.joblib')
            print('found %d frames in %s (dset=%s)' % (len(self.full_idxs), dataset_location, dset))
    ...
    def _get_views(self, index, num, resolution, rng):
        ...
        if num != 1:
            ...
            rest_frame = self.rank[anchor_frame][1:top_k]
            if self.specify:
                L = len(rest_frame)
                ...
                rest_frame_indexs = np.random.choice(rest_frame, size=num-1, replace=False).tolist()
...
if __name__ == "__main__":
    ...
    dataset = Dynamic_Replica(
        dataset_location="/mnt/disk3.8-4/datasets/dynamic_replica",
        dset='test',
        use_cache=False,
        use_augs=use_augs,
        top_k=50,
