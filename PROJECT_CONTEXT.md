...
???????????? `frame 4` ??????????? `LiveReplayViewer` ??????????????????????? `state.rgb` ????????????????? `export_canvas_pointcloud` ?????????????????????????? `delta_mask`????????????????????????????????

![????????????????????](assets/dataflow/live_viewer_frame4.png)

?????????????????????

1. ???? `state.rgb` ? `[0,1]` ?? `[0,255]` ? `uint8`??? RGB ?? OpenCV ??? BGR?`valid=false` ??????????
2. pipeline ???????????`pipeline_edge = pipeline_mask & ~erode(pipeline_mask, 2)`?
3. ?????????????????`delta_edge = delta_mask & ~erode(delta_mask, 2)`??? RGB ????????????????????
4. ?????? `depth/rgb/valid` ????????????????? canonical canvas??? `changed` ????? `3?3` ??????????????????
5. ????????????????`max_points=0`??????? `points` ????????`clean` ????????????????????`delta` ??????????? canonical cell ??
