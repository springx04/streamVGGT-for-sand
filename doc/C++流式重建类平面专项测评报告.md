...
资源采样的重复运行使用相同配置，第二次 5 帧总计为 5,446.351 ms，即 0.918 FPS；两次吞吐相差约 5.13%，且跳帧行为和点数结果一致。因此当前机器在本次固定 pair-letterbox 配置下的可复现实用速度可写成约 0.92–0.97 FPS，而不是只引用一次运行的单点值。对于旋转观测孔这类相对缓慢的采集过程，该速度配合 40% 跳帧可以有效降低重复计算；它不应表述为 30 FPS 实时视频速度。

#### 5.3.2 动态尺寸迁移状态

本次 C++ 速度结果有一个重要限定：动态 ROI 裁剪已经生效，但动态尺寸的 TorchScript pair 推理没有在本次命令中启用。

| 项目 | 当前 C++ 测试 | Python 动态 ROI 参考 |
|---|---|---|
| 后续帧 ROI | 帧 1 为 490×700，帧 2 为 518×700 | 项目记录中的帧 1 为 672×700，变化较大帧可为 560×700 |
| 实际模型输入 | 帧 1/2 均为 700×700 | 真实双帧输入随 ROI 使用 700×672、700×560 等形状 |
| 启用方式 | --pair-letterbox，固定 700×700 pair 图 | Python 侧按精确 ROI bucket 运行 |
| 本地 artifact | 只有 700×434 首帧图和 700×700 pair 图 | Python 侧直接使用动态形状 |

因此，当前后续模型帧之所以达到 1,698.894–1,788.603 ms，主要原因是它们仍然执行固定的双帧 700×700 图；ROI 虽然变小了，但模型 token/输入面积没有同步变小。首帧则只执行单帧 700×434 图，二者不是同一计算量。

C++ 代码中已经存在动态 pair 的运行分支：--model-pair-dir 会按当前 ROI 的精确宽高选择并加载对应的 WxH TorchScript artifact；项目也提供了 setc/scripts/export_pair_bucket_family.py 用于生成这组图。但本次 setc/artifacts 中没有完整的动态 bucket family，且测试命令使用了 --pair-letterbox，所以动态 pair 路径属于“代码已接入、artifact 尚未配齐、尚未完成速度实测”的状态。

这也解释了为什么 Python 旧测试中模型阶段平均值可以低于首帧，而当前 C++ 测试中后续执行帧反而更慢。即便启用动态 pair，后续帧仍包含双帧模型、单应对齐和融合开销，因此应分别报告 model_ms 与 total_ms，不能只根据点数减少就保证整帧一定比首帧快。当前 0.92–0.97 FPS 应标注为固定 pair-letterbox 基线，不能作为动态 bucket 迁移完成后的最终速度结论。
