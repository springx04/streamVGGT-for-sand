...
本附录单独记录已有的 Python 版 live replay 结果，用来说明动态 ROI 和模型推理时间的参考上限。数据来源为 `stream_omnivggt_outputs/data2_python_live_replay/timings.md`，对应 12 帧序列；它不与正文中的 C++ 固定 `pair-letterbox` 结果混合统计。
...
Python 版的输入形状不是固定的：项目记录显示 frame 1 使用 `700×672` 的双帧输入，frame 4 使用 `560×700` bucket。这正是本项目后续需要迁移到 C++ TorchScript pair 路径并实测的部分。
...
frame 11 未执行模型，仅维护流式状态，因此它的 `model_ms=0`，不能作为动态 ROI 推理速度样本。Python 结果的正确解读是：动态 ROI 让模型阶段明显缩短；整帧吞吐仍由几何对齐、融合和跳帧策略共同决定。该结果也构成后续 C++ 动态 bucket 复测的对照基线。
