...
struct GroupWorldView {
    ...
    // Prediction FOVs are model-space radians for world-point reprojection.
    // They are intentionally per-view; no fixed camera intrinsics are used.
    float fov_h = 0.0f;
    float fov_w = 0.0f;
    bool has_pose = false;
};
...
private:
    ...
    std::uint64_t accepted_fuse_count_ = 0;
