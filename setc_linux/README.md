...
（`1-1`、`1-2`、`1-3`，然后 `2-1`、`2-2`、`2-3`、…），组内锚点为中图，一次
 `B=1,S=3` CUDA BF16 forward。三相机输入保持原始宽高比并中心裁剪到模型尺寸，不再直接拉伸；下方低纹理区域经过稳健曲面拟合后独立做稠密 XY 栅格压缩。三个相机先独立栅格化，利用真实重叠点校正逐通道曝光差，再按置信度和到各视图边界的距离进行羽化融合；模型边缘低可靠点会被裁掉，不对未观测底面做外推补点。XY atlas 使用画布短边做等比例世界坐标定标，避免 770x630 画布把有效 Y 范围裁成水平直边。机械臂和其他非平面点使用独立槽位、只保留锚相机近地观测并过滤空间孤岛，避免与底面竞争或产生多机械臂重影。组模型输入 `406x252`，700x700 对齐画布、队列
